...
function Disciplinario() {
    ...
    const getDisciplinario = async () => {
        try {
         let json = await fetcher(urls.disciplinario.url);
         setData(json);
       } catch (error) {
         console.error(error);
       } finally {
    ...
    return (
      <View style={[listStyle.mainContainer, {height: '100%'}]}>
        <Text style={[listStyle.categoryText, {marginHorizontal: 15, marginBottom: 10}]}>{"Requisiti per le Strutture Ricettive per la classificazione Bike Hospitality"}</Text>
                ...
                <Card style={[listStyle.itemCardLeftImage, {margin: 10, alignContent: 'center'}]} >
                  <View style={detailStyle.sectionView}>
                      <View style={detailStyle.flexDirectionRow}>
                          ...
                          <Ionicons name={Platform.OS === "ios" ? "ios-information-circle" : "md-information-circle"} size={30} color={(!item.titolo.includes("obbl") ? 'green' : 'red')}/>
                      </View>
